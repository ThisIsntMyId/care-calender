...
export default function DoctorDashboardPage() {
  ...
  const [activeTab, setActiveTab] = useState<'today' | 'scheduled' | 'unscheduled' | 'completed'>('today');
  ...
  const handleMarkAsCompleted = async (taskId: number) => {
    try {
      const response = await fetch('/api/doctor/task/complete', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        ...
        <div className="bg-white rounded-lg shadow-md overflow-hidden">
          ...
          <div className="border-b border-gray-200">
            <div className="flex">
              ...
              >
                Completed {activeTab === 'completed' && (tasksLoading ? '(...)' : `(${tasks.length})`)}
              </button>
            </div>
          </div>
          ...
          <div className="px-6 py-4 bg-gray-50 border-b border-gray-200 flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-600 mt-1">
                ...
                {activeTab === 'completed' && 'Appointments that have been completed'}
              </p>
            </div>
            {activeTab === 'scheduled' && (
